...
void drawKid()
{
  if (kid.isActive)
  {
    ...
    if (!kid.isSucking)
    {
      ...
      sprites.drawOverwrite(kidcam.x, kidcam.y, kidSprite, kid.frame + 7 * kid.direction + ((kid.isJumping << 2) + 5 * (kid.isLanding || kid.isBalloon)) * !kid.isSucking + kid.isClimbing*6 );
    }
